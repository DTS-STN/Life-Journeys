...
export function SideMenu({ title, links }) {
  ...
  return (
    <div id="Sidemenu" className="m-2 p-4 sticky top-0 block max-w-sm float-left text-white">
          ...
          {links.map(({ url, text }, index) => (
            <li key={index} className="m-0 p-0">
              <Link href={url} scroll={false} >
                <a className="menuLink text-white" href={url} data-text={`${text}`}>
                  {text}
                </a>
              </Link>
            </li>
...
SideMenu.defaultProps = {
  ...
  links: [{ url: "#GettingReady", text: "Getting ready" },
